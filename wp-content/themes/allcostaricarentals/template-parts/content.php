...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header">
		<?php
		if ( is_singular() ) :
			the_title( '<h1 class="entry-title font-titles">', '</h1>' );
		else :
			the_title( '<h2 class="entry-title font-titles"><a href="' . esc_url( get_permalink() ) . '" rel="bookmark">', '</a></h2>' );
		endif;
		...
		if ( 'post' === get_post_type() ) :
			...
			<div class="entry-meta">
				<?php
				allcostaricarentals_posted_on();
				allcostaricarentals_posted_by();
				?>
			</div><!-- .entry-meta -->
		<?php endif; ?>
	</header><!-- .entry-header -->
	...
	<div class="entry-content">
		...
		the_content( sprintf(
			wp_kses(
				...
				__( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'allcostaricarentals' ),
				array(
					'span' => array(
						'class' => array(),
					),
	...
	<footer class="entry-footer">
		<?php allcostaricarentals_entry_footer(); ?>
	</footer><!-- .entry-footer -->
